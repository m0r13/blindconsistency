...
PATCHBITMAP *greyscale16(PATCHBITMAP *a) {
  ...
  PATCHBITMAP *ans = create_bitmap_ex(16, a->w, a->h);
  for (int y = 0; y < a->h; y++) {
...
PATCHBITMAP *greyscale(PATCHBITMAP *a) {
  if (bitmap_color_depth(a) != 32) { fprintf(stderr, "Input color depth for greyscale not 32\n"); exit(1); }
  PATCHBITMAP *ans = create_bitmap_ex(8, a->w, a->h);
  for (int y = 0; y < a->h; y++) {
    int *in_row = (int *) a->line[y];
...
PATCHBITMAP *greyscale16_to_color(PATCHBITMAP *a) {
  ...
  if (sizeof(unsigned short) != 2) { fprintf(stderr, "Size of unsigned short != 2\n"); exit(1); }
  PATCHBITMAP *ans = create_bitmap_ex(32, a->w, a->h);
  for (int y = 0; y < a->h; y++) {
...
PATCHBITMAP *horiz_gaussian_blur(PATCHBITMAP *a, double sigma) {
...
PATCHBITMAP *transpose16(PATCHBITMAP *bmp) {
  if (bitmap_color_depth(bmp) != 16) { fprintf(stderr, "Bitmap color depth for transpose16 not 16\n"); exit(1); }
  if (sizeof(unsigned short) != 2) { fprintf(stderr, "Size of unsigned short != 2\n"); exit(1); }
  PATCHBITMAP *ans = create_bitmap_ex(16, bmp->h, bmp->w);
...
PATCHBITMAP *gaussian_blur16(PATCHBITMAP *a, double sigma) {
  PATCHBITMAP *temp = horiz_gaussian_blur(a, sigma);
  PATCHBITMAP *tempT = transpose16(temp);
  destroy_bitmap(temp);
  PATCHBITMAP *ans = horiz_gaussian_blur(tempT, sigma);
  destroy_bitmap(tempT);
  PATCHBITMAP *ansT = transpose16(ans);
  destroy_bitmap(ans);
  return ansT;
}
...
PATCHBITMAP *color_gaussian_blur(PATCHBITMAP *a, double sigma, int aconstraint_alpha) {
  ...
  PATCHBITMAP *r = create_bitmap_ex(16, a->w, a->h);
  PATCHBITMAP *g = create_bitmap_ex(16, a->w, a->h);
  PATCHBITMAP *b = create_bitmap_ex(16, a->w, a->h);
  PATCHBITMAP *alpha = aconstraint_alpha ? create_bitmap_ex(16, a->w, a->h): NULL;
  for (int y = 0; y < a->h; y++) {
    for (int x = 0; x < a->w; x++) {
  ...
  PATCHBITMAP *rp = gaussian_blur16(r, sigma);
  PATCHBITMAP *gp = gaussian_blur16(g, sigma);
  PATCHBITMAP *bp = gaussian_blur16(b, sigma);
  PATCHBITMAP *alphap = aconstraint_alpha ? gaussian_blur16(alpha, sigma): NULL;
  PATCHBITMAP *ans = create_bitmap(a->w, a->h);
  for (int y = 0; y < a->h; y++) {
    for (int x = 0; x < a->w; x++) {
      ((int *) ans->line[y])[x] = 
...
PATCHBITMAP *gaussian_deriv_angle(PATCHBITMAP *a, double sigma, PATCHBITMAP **gx_out, PATCHBITMAP **gy_out) {
  fprintf(stderr, "gaussian_deriv_angle unimplemented\n"); exit(1);
#if 0
  PATCHBITMAP *b = gaussian_blur16(a, sigma);
...
PRINCIPAL_ANGLE *create_principal_angle(Params *p, PATCHBITMAP *bmp) {
  ...
  PATCHBITMAP *bmp16 = greyscale16(bmp);
...
PATCHBITMAP *extract_vbmp(VBMP *bmp, int i) {
  ...
  PATCHBITMAP *ans = create_bitmap(bmp->w, bmp->h);
  for (int y = 0; y < bmp->h; y++) {
    for (int x = 0; x < bmp->w; x++) {
      int c = bmp->get(x, y)[i];
      ((int *) ans->line[y])[x] = c;
...
VBMP *knn_init_nn(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *&ann_sim, PRINCIPAL_ANGLE *pa) {
  init_xform_tables();
...
void knn_attempt_n(vector<qtype<int> > &q, int *adata, PATCHBITMAP *b, XFORM bpos, int bx, int by, int bs, int bt, Params *p, int dval_known, PositionSet &pos0) {
  //int h = PATCH_W/2;
...
VBMP *knn_init_dist_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim) {
  init_xform_tables();
...
PATCHBITMAP *dprev = NULL;

void normalize_bitmap(PATCHBITMAP *ans) {
  int dmax = 0;
  for (int y = 0; y < ans->h; y++) {
    int *row = (int *) ans->line[y];
    for (int x = 0; x < ans->w; x++) {
...
void save_dist(Params *p, VBMP *annd, const char *suffix) {
...
#if 0
  PATCHBITMAP *ans = create_bitmap(annd->w, annd->h);
  for (int y = 0; y < ans->h; y++) {
    int *row = (int *) ans->line[y];
    for (int x = 0; x < ans->w; x++) {
  ...
  if (dprev) {
    PATCHBITMAP *delta = create_bitmap(annd->w, annd->h);
    for (int y = 0; y < ans->h; y++) {
...
void change_knn(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *&ann0, VBMP *&ann_sim0, VBMP *&annd0, int kp, PRINCIPAL_ANGLE *pa) {
  if (kp == p->knn) { return; }
...
void knn_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
...
template<int PATCH_W>
void knn_n_top1nn(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
            VBMP *ann0, VBMP *ann_sim0, VBMP *annd0,
            RegionMasks *amask=NULL, PATCHBITMAP *bmask=NULL,
            int level=0, int em_iter=0, RecomposeParams *rp=NULL, int offset_iter=0, int update_type=0, int cache_b=0,
            RegionMasks *region_masks=NULL, int tiles=-1, PRINCIPAL_ANGLE *pa=NULL, int save_first=1) {
  if (ann_sim0) { fprintf(stderr, "Unimplemented for rotation+scale\n"); exit(1); }
  PATCHBITMAP *ann = create_bitmap(ann0->w, ann0->h);
  PATCHBITMAP *annd = create_bitmap(annd0->w, annd0->h);
...
template<int PATCH_W>
void knn_n_window(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
            VBMP *ann, VBMP *ann_sim, VBMP *annd,
            RegionMasks *amask=NULL, PATCHBITMAP *bmask=NULL,
            int level=0, int em_iter=0, RecomposeParams *rp=NULL, int offset_iter=0, int update_type=0, int cache_b=0,
            RegionMasks *region_masks=NULL, int tiles=-1, PRINCIPAL_ANGLE *pa=NULL, int save_first=1) {
...
template<int PATCH_W>
void avoid_attempt_n(int &err, int &xbest, int &ybest, int &sbest, int &tbest, int *adata, PATCHBITMAP *b, XFORM bpos, int bx, int by, int bs, int bt, Params *p, PositionSet *avoid, int nn_i) {
...
template<int PATCH_W>
void knn_n_avoid(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
            VBMP *ann, VBMP *ann_sim, VBMP *annd,
            RegionMasks *amask=NULL, PATCHBITMAP *bmask=NULL,
            int level=0, int em_iter=0, RecomposeParams *rp=NULL, int offset_iter=0, int update_type=0, int cache_b=0,
...
template<int PATCH_W>
void knn_min_pair_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *annd, VBMP *ann_sim, VBMP *ann_temp, VBMP *annd_temp, VBMP *ann_sim_temp) {
...
void knn_min_pair(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann0, VBMP *annd0, VBMP *ann_sim0, VBMP *ann_temp, VBMP *annd_temp, VBMP *ann_sim_temp) {
...
void knn_oneway(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
         VBMP *ann, VBMP *ann_sim, VBMP *annd,
         RegionMasks *amask, PATCHBITMAP *bmask,
...
#if VOTE_SUM
PATCHBITMAP *knn_norm_image(Params *p, double *accum, int w, int h, PATCHBITMAP *b) {
#else
PATCHBITMAP *knn_norm_image(Params *p, vector<pair<int, int> > *accum, int w, int h, PATCHBITMAP *b) {
#endif
  PATCHBITMAP *ans = create_bitmap(w, h);
...
PATCHBITMAP *knn_vote_n(Params *p, PATCHBITMAP *b,
                 VBMP *ann, VBMP *ann_sim, VBMP *annd, VBMP *bnn, VBMP *bnn_sim,
                 PATCHBITMAP *bmask, PATCHBITMAP *bweight,
                 double coherence_weight, double complete_weight,
                 RegionMasks *amask, PATCHBITMAP *aweight, PATCHBITMAP *ainit, RegionMasks *region_masks, PATCHBITMAP *aconstraint, int mask_self_only, KNNWeightFunc *weight_func, double **accum_out) {
  FILE *fout = NULL;
  if (write_weights) {
  ...
  int y_last_clear = 0;
  PATCHBITMAP *ans = do_clear ? create_bitmap(ann->w, ann->h): NULL;
/*  if (accum_out) {
...
#pragma omp parallel for schedule(static, 8)
  ...
  for (int y = 0; y < ann->h-p->patch_w+1; y++) {
    ...
    if (do_clear) {
      ...
      if (y_current_clear > y_last_clear) {
        PATCHBITMAP *subb = create_sub_bitmap(b, 0, y_last_clear, b->w, y_current_clear-y_last_clear);
#if VOTE_SUM
        PATCHBITMAP *subans = knn_norm_image(p, &accum[4*(y_last_clear*ann->w)], ann->w, (y_current_clear-y_last_clear), subb);
#else
        PATCHBITMAP *subans = knn_norm_image(p, &accum[3*(y_last_clear*ann->w)], ann->w, (y_current_clear-y_last_clear), subb);
#endif
        destroy_bitmap(subb);
  ...
  PATCHBITMAP *ans = norm_image(accum, ann->w, ann->h);
  delete[] accum;
  return ans;
#endif
...
//double psnr(PATCHBITMAP *a, PATCHBITMAP *b);
...
class KNNSolver: public ObjectiveFunc { public:
  ...
  int n;
  PATCHBITMAP *aorig;
  KNNSolver(Params *p_, PATCHBITMAP *b_, VBMP *ann_, VBMP *ann_sim_, VBMP *annd_, int n_, PATCHBITMAP *aorig_) {
    p = p_;
    b = b_;
    ann = ann_;
    ann_sim = ann_sim_;
  ...
  virtual double f(double x[]) {
    //PATCHBITMAP *a = knn_vote(p, a, ann, ann_sim, annd, NULL, NULL, 0, 0, 0.5, 0.5, weight_func);
    KNNSolverWeightFunc weight_func(x);
    PATCHBITMAP *a = knn_vote(p, b, ann, ann_sim, annd, NULL, NULL, NULL, NULL, COHERENCE_WEIGHT, COMPLETE_WEIGHT, NULL, NULL, NULL, NULL, NULL, 0, &weight_func);
...
template<int PATCH_W>
void knn_enrich_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
  double enrich_start_t = accurate_timer();
...
void knn_enrich(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
...
template<int PATCH_W>
void knn_enrich3_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
  if (a != b) { fprintf(stderr, "knn_enrich unimplemented for a != b\n"); exit(1); }
#if !TRANSLATE_ONLY
  fprintf(stderr, "knn_enrich unimplemented for rotation+scale mode\n"); exit(1);
#endif
...
void knn_enrich3(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
...
template<int PATCH_W>
void knn_enrich4_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
  if (a != b) { fprintf(stderr, "knn_enrich unimplemented for a != b\n"); exit(1); }
#if !TRANSLATE_ONLY
  fprintf(stderr, "knn_enrich unimplemented for rotation+scale mode\n"); exit(1);
#endif
...
void knn_enrich4(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
...
template<int PATCH_W>
void knn_inverse_enrich_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
...
void knn_inverse_enrich(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
...
template<int PATCH_W>
void knn_inverse_enrich2_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
...
void knn_inverse_enrich2(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
...
template<int PATCH_W>
void knn_check_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, int check_duplicates) {
...
void knn_check(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, int check_duplicates) {
...
extern PATCHBITMAP *buffer;
extern PATCHBITMAP *mouse;
...
PATCHBITMAP *sample_xform(Params *p, PATCHBITMAP *a, XFORM bpos) {
  PATCHBITMAP *ans = create_bitmap(p->patch_w, p->patch_w);
...
void knn_vis(Params *p, PATCHBITMAP *a, VBMP *ann0, VBMP *ann_sim0, VBMP *annd0, int is_bitmap, PATCHBITMAP *vote, PATCHBITMAP *orig, PATCHBITMAP *vote_uniform) {
  ...
  PATCHBITMAP *buades = NULL; //load_image("flowers_denoise_buades7.bmp");
   ...
   // printf("in knn_vis\n");
    ...
    if ((unsigned) mx < (unsigned) AEW && (unsigned) my < (unsigned) AEH) {
      ...
      if (!is_bitmap) {
        ...
        p_ann     = ((int *) ((PATCHBITMAP *) ann    )->line[my])+mx;
#if !TRANSLATE_ONLY
        p_ann_sim = ((int *) ((PATCHBITMAP *) ann_sim)->line[my])+mx;
#endif
        p_annd    = ((int *) ((PATCHBITMAP *) annd   )->line[my])+mx;
      ...
      for (int i = 0; i < p->knn; i++) {
        ...
        if (dovis) {
          ...
          //XFORM bpos = get_xform(p, xpp, ypp, spp, tpp);
          PATCHBITMAP *sampled = sample_xform(p, a, bpos);
...
PATCHBITMAP *visnn(Params *p, PATCHBITMAP *ann, int is_rel, PATCHBITMAP *b);
...
double smooth_energy_for(Params *p, PATCHBITMAP *a, VBMP *ann, int *ans, int x1, int y1, int x2, int y2) {
...
double data_energy_for(Params *p, PATCHBITMAP *a, int *ans, int i, VBMP *annd) {
  int *p_annd = annd->get(i%AEW, i/AEW);
  return data_energy(ans[i], p_annd[ans[i]]);
}

/* Simple greedy pixel-changing algorithm. */
PATCHBITMAP *knn_smooth2(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PATCHBITMAP *&annd_out) {
  ...
  PATCHBITMAP *ann_out = create_bitmap(a->w, a->h);
  clear(ann_out);
...
VBMP *bitmap_to_vbmp(PATCHBITMAP *bmp) {
...
PATCHBITMAP *visd(Params *p, PATCHBITMAP *annd);

void smooth_coherent(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *ann, PATCHBITMAP *annd, PATCHBITMAP *&ann_out, PATCHBITMAP *&annd_out, int nsmooth) {
